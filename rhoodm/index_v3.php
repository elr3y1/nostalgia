...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Obtener el título del playlist (si es NA es un track individual)
    $playlistTitle = trim(shell_exec("\"$yt_dlp\" --print \"%(playlist_title)s\" " . escapeshellarg($url)));

    // Definir carpeta y patrón de salida
    if ($playlistTitle === 'NA') {
        // Track individual
        $folderName = "One Hit Wonder";
        $outputPattern = "$downloadDir$folderName/%(artist)s - %(title)s.%(ext)s";
        $finalName = "One Hit Wonder";

        // Descargar carátula del track
        $thumbnailCmd = "\"$yt_dlp\" --skip-download --write-thumbnail --convert-thumbnails jpg -o \"$downloadDir$folderName/track_thumb.%(ext)s\" " . escapeshellarg($url);
        exec($thumbnailCmd);
    } else {
        // Playlist
        $folderName = $playlistTitle;
        $outputPattern = "$downloadDir$folderName/%(playlist_index)02d - %(title)s.%(ext)s";
        $finalName = $playlistTitle;

        // Descargar carátula de la playlist
        $thumbnailCmd = "\"$yt_dlp\" --skip-download --playlist-items 1 --write-thumbnail --convert-thumbnails jpg -o \"$downloadDir$folderName/playlist_thumb.%(ext)s\" " . escapeshellarg($url);
        exec($thumbnailCmd);
    }
...
<script>
  ...
  $('#downloadForm').on('submit', function(e) {
    ...
    Swal.fire({
      title: 'Procesando...',
      text: 'Descargando MP3 y carátula (' + quality + ' kbps)',
      icon: 'info',
